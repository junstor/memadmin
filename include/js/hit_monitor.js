$(function() {
    ...
    $("#scons").change(function() {
        window.location.href = "memmanager.php?type=" + type + "&num=" + num + "&action=hitmonitor&conid=" + $(this).val();
    });
    $("#gomonitorbut").click(function() {
        ...
        $.ajax({
            ...
            success: function(d) {
                if (d == 'OK') {
                    window.location.href = "show_monitor_hit.php";
                    return;
                } else {
                    alert('Fail');
                    return;
...
function check2json() {
    var jsonstr = "{'data':[{'monitor':'hit','type':'" + type + "','num':'" + num + "','conid':'";
    if (type == 'con') 
      jsonstr += "0'},[";
    else if (type == 'conp') 
      jsonstr += $("#scons").val() + "'},[";
